...
class CpGIslandDataset(Dataset):
    def __init__(self, root_dir, augment=False):
        ...
        # 检查并加载one_hot_encodings.h5
        one_hot_path = os.path.join(root_dir, 'one_hot_encodings.h5')
        if not os.path.exists(one_hot_path):
            raise FileNotFoundError(f"找不到文件: {one_hot_path}")

        # 加载one_hot数据，我们的h5文件中，由于已经分了三个集，每个集中的h5键值只有一个
        ...
        if not os.path.exists(features_path):   # 我们的数据处理和使用过程中，这个文件都能保证有，但是为了代码的鲁棒性，添加了这个语句
            print("警告: 找不到原始特征文件 features.csv，将不使用原始特征")
            self.features = None
            self.original_feature_dim = 0
        else:
            self.features = pd.read_csv(features_path).values
            # 检查并处理NaN和Inf值，由于没有Inf值，所以cnn模型代码中省略了Inf的情况
            ...
            print(f"原始特征维度: {self.original_feature_dim}") # 便于提前了解特征的维度数
